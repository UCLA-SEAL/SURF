...
def convert(vertmap_path:str, edge_map_path:str, graph_path:str, focus_verts:str):
    ...
    with open(graph_path, 'r') as graph_file:
        ...
        for line in graph_file:
            ...
            if line.startswith('t'):
                ...
                splitted = line.split(' ')
                content[current_id] += "e " + splitted[1] + " " + splitted[2] + " " + edge_map[int(splitted[3])] + "\n"
    ...
    for item in sorted_rows:
        ...
        # we only print if the focus node is in the graph

        intersection = set(focus_verts).intersection(set(nodes.values()))
        if not show_only_order and len(intersection) == 0:
