...
def guess_path_to_best_subgraph_file(API:str) -> Path:
    ...
    file_name = API + "_formatted_onlyunlabelled_" + experiment_id + str(request_counter) + "_result_best_subgraphs.txt"
...
def path_to_weighted_nodes_file(API:str) -> str:
    base_dir = project_path + "code/graphs/"
    file_name = API + "_formatted_onlyunlabelled_" + experiment_id + str(request_counter) + "_result_weighted_nodes.txt"
...
def read_features_file(API):
    ...
    feature_ids = []
    with open(project_path + "code/graphs/" + API + "_formatted_onlyunlabelled_" + experiment_id + str(request_counter) + "_result_features.txt", 'r') as f:
        for line in f:
            line = line.strip()
...
def debug2():
    ...
    if API_under_test.endswith("true"):
        raise Exception("accidentally ended API's name with true. you probably used the wrong file?")
